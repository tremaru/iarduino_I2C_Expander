...
//  Проходим по всем доступным адресам на шине I2C:               //
    ...
    j=Wire.endTransmission();                                     // Выполняем инициированную ранее передачу данных.
      if(j==0){                                                   // Если передача выполнена успешно (0-передача успешна / 1 - переполнен буфер для передачи / 2 - получен NACK при передаче адреса / 3 - получен NACK при передаче данных / 4 - другая ошибка), то ...
        ...
        if( ((k[2]>>1)==i) && k[3]==0xC3 ){                       // Если значение второго элемента массива k совпадает с адресом устройства, а в третьем элементе хранится идентификатор 0xC3 хначит это светодиодная матрица или модуль метро.
          ...
          if(k[0]==5){Serial.print("датчиком DHT");        }else  // Если в нулевом элементе массива k хранится значение 5, значит это датчик владности и температуры.
